...
function setupBot(bot: Bot) {
  ...
  bot.command('stats', async (ctx) => {
    ...
    const { data: user } = await supabaseAdmin
      .from('users').select('*').eq('telegram_chat_id', chatId).single();

    if (!user) {
      await ctx.reply('⚠️ Аккаунт не привязан.\nПривяжите Telegram в настройках: eseo-ai.ru/dashboard/settings');
      return;
    }
    ...
    const { count: reviewsCount } = await supabaseAdmin
      .from('reviews').select('*', { count: 'exact', head: true }).eq('user_id', user.id).eq('reply_status', 'published');
  ...
  bot.command('link', async (ctx) => {
    ...
    await supabaseAdmin.from('activity_log').insert({ action: 'telegram_link_request', details: { chat_id: chatId, code } });
    await ctx.reply(`🔗 Код привязки: \`${code}\`\n\nВведите этот код на странице:\neseo-ai.ru/dashboard/settings → Telegram`, { parse_mode: 'Markdown' });
  });

  bot.callbackQuery('my_cards', async (ctx) => {
    await ctx.answerCallbackQuery();
    const chatId = ctx.chat?.id;
    const { data: user } = await supabaseAdmin.from('users').select('id').eq('telegram_chat_id', chatId).single();
    if (!user) { await ctx.reply('⚠️ Привяжите аккаунт через /link'); return; }
    const { data: cards } = await supabaseAdmin.from('cards').select('title, marketplace, seo_score, created_at').eq('user_id', user.id).order('created_at', { ascending: false }).limit(5);
    if (!cards?.length) { await ctx.reply('У вас пока нет карточек. Создайте первую через /generate'); return; }
    const list = cards.map((c, i) => `${i + 1}. [${c.marketplace.toUpperCase()}] ${c.title?.slice(0, 40)}... (SEO: ${c.seo_score})`).join('\n');
    await ctx.reply(`📋 *Последние карточки:*\n\n${list}`, { parse_mode: 'Markdown' });
  });

  bot.callbackQuery('pricing', async (ctx) => {
    await ctx.answerCallbackQuery();
    const keyboard = new InlineKeyboard().url('Перейти к тарифам', 'https://eseo-ai.ru/pricing');
    await ctx.reply('💰 *Тарифы ESEO*\n\n🆓 Starter — 0 ₽\n• 5 карточек/мес\n\n⭐ Pro — 29 000 ₽/мес\n• Безлимит + аналитика + автоответы\n\n🏢 Enterprise — от 79 000 ₽/мес\n• White Label + выделенный менеджер', { parse_mode: 'Markdown', reply_markup: keyboard });
  });
